...
class Repo(private val db: AppDatabase) {
    ...
    suspend fun markToday(went: Boolean, today: LocalDate = LocalDate.now()) {
        attendance.upsert(Attendance(today, went))
    }

    suspend fun setAttendance(date: LocalDate, went: Boolean) {
        attendance.upsert(Attendance(date, went))
    }

    suspend fun deleteAttendance(date: LocalDate) {
        attendance.deleteByDate(date)
    }
    ...
    companion object {
        fun get(context: android.content.Context) = Repo(AppDatabase.get(context))
    }
}